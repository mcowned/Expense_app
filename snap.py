...
class HomePage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        button = ttk.Button(self, text="  Sales", width=10, image=self.document, compound=LEFT, command=lambda: controller.show_frame(Sales))
        button.grid(row=1, column=1, padx=25, pady=25, ipady=5)
 
        button2 = ttk.Button(self, text="  Owing", width=10, image=self.bookmark, compound=LEFT, command=lambda: controller.show_frame(Owing))
        button2.grid(row=1, column=2, padx=25, pady=25, ipady=5)

        button3 = ttk.Button(self, text="  Summary", width=10, image=self.info, compound=LEFT, command=lambda: controller.show_frame(Summary))
        button3.grid(row=1, column=3, padx=25, pady=25, ipady=5)

        button4 = ttk.Button(self, text="  Expense", width=10, image=self.expense, compound=LEFT, command=lambda: controller.show_frame(Expense))
        button4.grid(row=2, column=1, padx=25, pady=25, ipady=5)
        
        button4 = ttk.Button(self, text="  Pricing", width=10, image=self.settings, compound=LEFT, command=lambda: controller.show_frame(Pricing))
        button4.grid(row=2, column=2, padx=25, pady=25, ipady=5)
        restartButton = ttk.Button(self, width=10, text="  Restart", image=self.restart, compound=LEFT, command=restart_program)
...
class Sales(tk.Frame):
    ...
    def create_widgets(self):
        ...
        # Enter Button
        self.enterButton = ttk.Button(self, width=10, image=self.check, compound=LEFT, text='  Enter', command=self.insert)
        self.enterButton.grid(row=4, column=0, ipady=5)

        # Refresh Button
        self.refreshButton = ttk.Button(self, width=10, image=self.restart, compound=LEFT, text='  Clear', command=self.clear)
        self.refreshButton.grid(row=6, column=0, ipady=5)

        # Remove Button
        self.removeButton = ttk.Button(self, width=10, image=self.trash, compound=LEFT, text='  Remove', command=self.remove)
        self.removeButton.grid(row=5, column=0, ipady=5)
        # Remove Button
        # self.testButton = ttk.Button(self, width=10, text='test', command=self.select_all)
        # self.testButton.grid(row=7, column=0)

        # Mitsi Entry
        self.mitsi_entry = tk.DoubleVar()
        # self.mitsi_entry = tk.StringVar()
        # self.mitsi_entry = tk.IntVar()
    ...
    def populateReportList(self):
        for self.row in db.reportfetch():
            self.tree.insert('', 0, text=self.row[0], values=(self.row[1], self.row[2], self.row[3], self.row[4], self.row[5], self.row[6], self.row[0]))
...
class Owing(tk.Frame):
    ...
    def owing_widgets(self):
        ...
        self.paidButton.grid(row=3, column=5, pady=25)
        ...
        self.accountCombo.grid(row=3, column=4)
...
class Summary(tk.Frame):
    ...
    def summary_widgets(self):
        ...
        # self.loan_labels = []
        # self.loan_totals = []
        # for k, v in db.loanfetch():
        #     self.loan_labels.append(k)
        #     v = f"{v:,d}"
        #     self.loan_totals.append(v)
...
class Expense(tk.Frame):
    ...
    def expense_widgets(self):
        ...
        # Combo box
        acc_values = ['Coin', 'Bills']
        self.categoryCombo = ttk.Combobox(self, width=15, values=self.expense_category, textvariable=self.category_combo)
        ...
        # Tree View
        self.expenseTree = ttk.Treeview(self, height=10, columns=('expense', 'amount', 'account', 'category', 'balances'))
    ...
    def populateExpenseList(self):
        for self.row in db.expensereport_fetch():
            self.expenseTree.insert('', 0, text=self.row[0], values=(self.row[1], self.row[2], self.row[3], self.row[4], self.row[5]))
    ...
    def enter(self):
        ...
        if self.account == "Coin":
            db.expense(self.expense, self.amount, self.account, self.category, self.coin_balance, self.final_CoinBalance, random.randint(1, 10000))
        elif self.account == "Bills":
            db.expense(self.expense, self.amount, self.account, self.category, self.bills_balance, self.final_BillsBalance, random.randint(1, 10000))
        self.refresh_list()
        print(f"Successfully added the data to the expensereport Database")
...
if __name__ == "__main__":
    app = Master()
    app.iconbitmap(r'images/money.ico')
